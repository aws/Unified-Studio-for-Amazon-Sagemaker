...
#   ./02-domain-account/scripts/deploy/01-deploy.sh               # full deploy
#   ./02-domain-account/scripts/deploy/01-deploy.sh --lambdas-only # Lambda code only
...
if [ "$LAMBDAS_ONLY" = "false" ]; then
    ...
    aws cloudformation deploy \
        --template-file 02-domain-account/templates/cloudformation/01-infrastructure.yaml \
...
if [ "$LAMBDAS_ONLY" = "false" ]; then
    echo "🔧 Step 2: Writing per-pool SSM parameters..."

    # Read org-side config (bucket, prefix) from org SSM — fall back to defaults
    # These are needed to enrich the stacksets JSON with S3 URLs and StackSet names
    TEMPLATE_BUCKET=$(aws ssm get-parameter \
        --name "/AccountPoolFactory/TemplateBucketName" \
        --region "$REGION" --query 'Parameter.Value' --output text 2>/dev/null || echo "")
    STACKSET_PREFIX_ORG=$(aws ssm get-parameter \
        --name "/AccountPoolFactory/StackSetPrefix" \
        --region "$REGION" --query 'Parameter.Value' --output text 2>/dev/null || echo "SMUS-AccountPoolFactory")

    # If domain account can't read org SSM directly, try old pool SSM params
    if [ -z "$TEMPLATE_BUCKET" ] || [ "$TEMPLATE_BUCKET" = "None" ]; then
        TEMPLATE_BUCKET="accountpoolfactory-templates-${ORG_ADMIN_ACCOUNT_ID}"
        echo "   ⚠️  Could not read TemplateBucketName from SSM, using default: $TEMPLATE_BUCKET"
    fi
    ...
    for pool_name in $POOL_NAMES; do
        ...
        eval "ou_name=\$POOL_${KEY}_OU_NAME"
        eval "ou_id=\$POOL_${KEY}_OU_ID"
        eval "stacksets_json=\$POOL_${KEY}_STACKSETS"

        # ou_id comes from config.yaml — set by org admin after org deploy
        if [ -z "$ou_id" ]; then
            echo "❌ ou_id not set for pool $pool_name in config.yaml"
            echo "   Run org deploy first, then set ou_id in 02-domain-account/config.yaml"
            exit 1
        fi

        echo "   Pool: $pool_name (OU: $ou_name → $ou_id)"

        # Enrich stacksets JSON with stacksetName and s3Url
        enriched_stacksets=$(python3 -c "
import json, re
prefix = '$STACKSET_PREFIX_ORG'
...
for entry in raw:
    ...
    result.append({
        'template': tmpl, 'stacksetName': stackset_name,
        'wave': entry['wave'], 's3Url': s3_url
    })
print(json.dumps(result))
")

        # Write per-pool SSM params
        ...
        aws ssm put-parameter --name "/AccountPoolFactory/Pools/${pool_name}/OUName" \
    ...
    # Legacy single-pool SSM params (backward compat)
...
# Setup Orchestrator: lambda + all approved CF templates
...
if [ "$LAMBDAS_ONLY" = "false" ]; then
    ...
    echo "  2. Org Admin runs: ./01-org-account/scripts/deploy/01-deploy.sh $DOMAIN_ACCOUNT_ID $DOMAIN_ID <provision-account-role-arn>"
    echo "  3. Then: ./02-domain-account/scripts/deploy/02-deploy-project-profile.sh"
    echo "  4. Then: ./02-domain-account/scripts/deploy/03-verify.sh"
fi
