...
cfg_file = os.path.join(PROJECT_ROOT, '02-domain-account/config.yaml')
...
if not DOMAIN_ID:
    print('❌ Could not resolve domain ID from 02-domain-account/config.yaml')
    sys.exit(1)

# Get pool config from first pool entry
pools = config.get('pools', [])
if not pools:
    print('❌ No pools defined in 02-domain-account/config.yaml')
    sys.exit(1)
first_pool = pools[0]
...
if args.delete:
    ...
    print(f'  Deleted. Monitor reclaim: python3 02-domain-account/scripts/utils/monitor-pool.py 10')
else:
