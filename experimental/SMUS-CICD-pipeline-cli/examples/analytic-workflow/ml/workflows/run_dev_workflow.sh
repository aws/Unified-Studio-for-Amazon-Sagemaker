...
echo "📤 Step 1: Upload ML scripts to S3"
run_command "aws s3 cp \"sagemaker_training_script.py\" \"s3://$BUCKET_NAME/scripts/sagemaker_training_script.py\" --region $REGION"
run_command "aws s3 cp \"model_evaluation.py\" \"s3://$BUCKET_NAME/scripts/model_evaluation.py\" --region $REGION"
run_command "aws s3 cp \"../scripts/tag_champion_model.py\" \"s3://$BUCKET_NAME/scripts/tag_champion_model.py\" --region $REGION"
