...
if aws iam get-role --role-name $ROLE_NAME >/dev/null 2>&1; then
    ...
    aws iam create-role \
        --role-name $ROLE_NAME \
        --assume-role-policy-document file://overdrive-trust-policy.json
    echo "✓ Role created"
fi
...
aws iam update-assume-role-policy \
    --role-name $ROLE_NAME \
    --policy-document file://overdrive-trust-policy.json
...
aws iam put-role-policy \
    ...
    --policy-document file://overdrive-sagemaker-policy.json
