...
def wait_for_data_source_runs_completion(domain_name, project_id, region, max_wait_seconds=300):
    ...
    try:
        ...
        print(f"Warning: Some data source runs still running after {max_wait_seconds} seconds")
...
def delete_project_custom_form_types(domain_name, project_id, region):
    ...
    try:
        ...
        for item in response.get('items', []):
            ...
            if (owning_project == project_id and 
                form_name.startswith('SageMakerUnifiedStudioScheduleFormType')):
        ...
        print(f"Warning: Error cleaning up custom form types for project {project_id}: {str(e)}")
        return []
    ...
    try:
        ...
        for data_source in data_sources_response.get('items', []):
            ...
            try:
                ...
                print(f"Warning: Failed to delete data source {data_source_name}: {str(e)}")
        
        return deleted_sources
        
    except Exception as e:
        print(f"Warning: Error deleting data sources for project {project_id}: {str(e)}")
        return []
    ...
    try:
        ...
        if deleted_environments:
            ...
            while time.time() - start_time < max_wait:
                ...
                print(f"Waiting for {len(remaining_envs)} environments to finish deleting...")
                time.sleep(10)
            else:
                print(f"Warning: Some environments still exist after {max_wait} seconds")
        
        return deleted_environments
        
    except Exception as e:
        print(f"Warning: Error deleting environments for project {project_id}: {str(e)}")
        return []
...
def delete_project(domain_name, project_id, region):
    ...
    try:
        ...
        deleted_environments = delete_project_environments(domain_name, project_id, region)
        if deleted_environments:
        ...
        deleted_forms = delete_project_custom_form_types(domain_name, project_id, region)
        if deleted_forms:
        ...
        try:
            ...
            if "MetaDataForms found" in str(e):
                print("Warning: Project deletion blocked by form types that cannot be deleted via API")
                ...
                raise Exception(f"Project deletion blocked by undeletable form types: {str(e)}")
            else:
                raise e
    ...
    try:
        ...
        try:
            ...
            return {
                ...
                'owners': ', '.join([
                    member.get('memberDetails', {}).get('user', {}).get('userIdentifier', 'Unknown')
