...
class ProjectManager:
    ...
    def _create_new_project(self, target_name: str, target_config, project_name: str) -> Dict[str, Any]:
        ...
        success = cloudformation.create_project_via_cloudformation(
            project_name, profile_name, self.domain_name, self.region,
            self.manifest.pipeline_name, target_name, target_config.stage, 
            user_parameters, owners, contributors
        )
    ...
    def _update_existing_project(self, target_name: str, target_config, project_name: str):
        ...
        cloudformation.update_project_stack_tags(
            project_name, self.domain_name, self.region,
            self.manifest.pipeline_name, target_name, target_config.stage
        )
    ...
    def _get_profile_name(self, target_config) -> str:
        ...
        if target_config.initialization and target_config.initialization.project:
            profile_name = profile_name or target_config.initialization.project.profile_name
        return profile_name
    
    def _extract_user_parameters(self, target_config, target_name: str) -> Optional[List]:
    ...
    def _build_user_parameters(self, yaml_user_params: List) -> List:
        ...
        from smus_cicd.pipeline.pipeline_manifest import EnvironmentUserParameters, UserParameter
        ...
        for env_config in yaml_user_params:
            ...
            user_parameters.append(EnvironmentUserParameters(
                EnvironmentConfigurationName=env_name,
                parameters=user_param_objects
            ))
    ...
    def _extract_memberships(self, target_config) -> tuple[List[str], List[str]]:
        ...
        if target_config.initialization and target_config.initialization.project:
            ...
            contributors = getattr(target_config.initialization.project, 'contributors', [])
        else:
