...
def create_airflow_serverless_client(connection_info: Dict[str, Any] = None, region: str = None):
...
def create_workflow(
    ...
    try:
        ...
        if 'ConflictException' in str(e) and 'already exists' in str(e):
            logger.info(f"Workflow {workflow_name} already exists, deleting and recreating workflow")
            ...
            try:
                ...
                logger.info(f"🔍 DEBUG: Delete workflow request params: {delete_params}")
                ...
                logger.error(f"Failed to delete and recreate workflow {workflow_name}: {delete_recreate_error}")
                raise Exception(f"Failed to delete and recreate workflow {workflow_name}: {delete_recreate_error}")
    ...
    try:
        ...
        for workflow in response.get('Workflows', []):
            workflow_data = {
                ...
                'name': workflow['Name'],
                'status': workflow['WorkflowStatus'],  # Use WorkflowStatus instead of Status
                'created_at': workflow['CreatedAt'],
                'updated_at': workflow['ModifiedAt'],  # Use ModifiedAt instead of UpdatedAt
                'tags': {}
            }
            
            # Fetch tags for each workflow
            try:
                tags_response = client.list_tags_for_resource(ResourceArn=workflow['WorkflowArn'])
                workflow_data['tags'] = tags_response.get('Tags', {})
            except Exception as tag_error:
                logger.warning(f"Failed to fetch tags for workflow {workflow['Name']}: {tag_error}")
                workflow_data['tags'] = {}
...
def start_workflow_run(
    ...
    try:
        ...
        return {
            'run_id': run_id,
            'workflow_arn': workflow_arn,
            'status': response.get('Status', 'QUEUED'),  # Default status if not provided
...
def get_workflow_run_status(
    ...
    try:
        ...
        return {
            'run_id': response['RunId'],
            'workflow_arn': response.get('WorkflowArn', workflow_arn),  # Use provided if not in response
            'status': status,
...
def get_workflow_status(
    ...
    try:
        ...
        return {
            'status': response['WorkflowStatus'],  # Use WorkflowStatus instead of Status
            'name': response['Name'],
            'created_at': response['CreatedAt'],
...
def wait_for_workflow_completion(
    ...
    while time.time() - start_time < timeout_seconds:
        status_result = get_workflow_run_status(workflow_arn, run_id, connection_info, region)
        ...
        if status in ['SUCCEEDED', 'FAILED', 'CANCELLED', 'SUCCESS', 'STOPPED', 'TIMEOUT']:
    ...
    logger.warning(f"Workflow run {run_id} did not complete within {timeout_minutes} minutes")
    return {
        'success': False,
