...
def validate_target_exists(target_name: str, manifest, context: str = "operation") -> None:
    ...
    if not manifest.get_target(target_name):
        ...
        handle_error(f"Target '{target_name}' not found in manifest for {context}. "
                    f"Available targets: {', '.join(available_targets)}")


def validate_required_config(config_value, config_name: str, context: str = "operation") -> None:
