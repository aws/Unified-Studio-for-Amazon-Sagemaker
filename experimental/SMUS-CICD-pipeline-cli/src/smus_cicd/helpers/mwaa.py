...
def get_environment_status(
    environment_name: str, region: str = None, connection_info: Dict[str, Any] = None
) -> Dict[str, Any]:
...
def delete_dag_from_history(
    ...
    try:
        ...
        raise Exception(f"Failed to delete DAG {dag_id} from MWAA environment {environment_name}: {e}")
...
def delete_multiple_dags_from_history(
    ...
    for dag_id in dag_ids:
        results[dag_id] = delete_dag_from_history(
            environment_name, dag_id, region, connection_info
        )
    return results
...
def get_airflow_ui_url(
    ...
    try:
        ...
        raise Exception(f"Failed to get webserver URL for MWAA environment {environment_name}: {e}")
...
def list_dags(
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if stdout_b64:
                ...
                for line in lines:
                    ...
                    if (
                        line
                        and not line.startswith("dag_id")
                        and not line.startswith("---")
                        and not line.startswith("===")  # Skip table separators
                        and not line.startswith("INFO")
                        and not line.startswith("WARNING")
                        and not line.startswith("[")  # Skip timestamp lines
                        and not line.startswith("AIRFLOW")
                        and "|" in line  # Must be a table row
                        and line != ""
                    ):
        ...
        raise Exception(f"Failed to list DAGs in MWAA environment {environment_name}: {e}")
...
def validate_mwaa_health(project_name: str, config: Dict[str, Any]) -> bool:
    ...
    try:
        ...
        raise Exception(f"MWAA health validation failed for project {project_name}: {e}")
