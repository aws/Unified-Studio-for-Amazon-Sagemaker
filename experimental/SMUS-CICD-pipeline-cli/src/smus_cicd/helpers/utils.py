...
def get_datazone_project_info(project_name: str, config: Dict[str, Any]) -> Dict[str, Any]:
    """
...
def _get_project_details(domain_id: str, project_id: str, region: str) -> Dict[str, Any]:
    ...
    return {
        'status': project_response.get('projectStatus', 'Unknown')
    }
...
def _get_project_connections(
    ...
    try:
        ...
        domain_name = config.get('domain', {}).get('name')
        if domain_name:
            return connections.get_project_connections(project_name, domain_name, region)
        else:
            return {}
    except Exception:
