    ...
    try:
        ...
        for target_name, target_config in targets_to_monitor.items():
            ...
            try:
                ...
                if project_info.get('project_id') and 'error' not in project_info:
                    ...
                    target_data["status"] = "active" if project_info.get('status') == 'ACTIVE' else "inactive"
                    ...
                    workflow_connections = {name: info for name, info in project_connections.items() 
                        ...
                        for conn_name, conn_info in workflow_connections.items():
                            ...
                            if env_name:
                                ...
                                try:
                                    ...
                                    existing_workflows = mwaa.list_dags(env_name, manifest.domain.region, conn_info)
                                    all_dag_details = mwaa.get_all_dag_details(env_name, manifest.domain.region, conn_info)
                                    ...
                                    if output.upper() != "JSON":
                                        ...
                                        # Get Airflow UI URL
                                        airflow_url = mwaa.get_airflow_ui_url(env_name, manifest.domain.region, conn_info)
                                        if airflow_url:
                                    ...
                                    if hasattr(manifest, 'workflows') and manifest.workflows:
                                        manifest_workflows = {w.workflow_name for w in manifest.workflows}
                                    ...
                                    for dag_name in sorted(existing_workflows):
                                        ...
                                        status = 'ACTIVE' if active else 'PAUSED' if active is False else 'UNKNOWN'
                                        
                                        # Get recent DAG runs for monitoring
                                        try:
                                            dag_runs = mwaa.get_dag_runs(dag_name, env_name, manifest.domain.region, conn_info, limit=5)
                                        ...
                                        if output.upper() != "JSON":
                                            status_icon = "✅" if recent_status == "success" else "❌" if recent_status == "failed" else "⏸️" if status == "PAUSED" else "🔄"
                                            manifest_icon = "✓" if dag_name in manifest_workflows else "-"
                                            
                                            typer.echo(f"         {status_icon} {manifest_icon} {dag_name}")
                                            typer.echo(f"            Schedule: {schedule} | Status: {status} | Recent: {recent_status}")
        ...
        if hasattr(manifest, 'workflows') and manifest.workflows and output.upper() != "JSON":
            ...
            for workflow in manifest.workflows:
                typer.echo(f"   - {workflow.workflow_name} (Connection: {workflow.connection_name})")
