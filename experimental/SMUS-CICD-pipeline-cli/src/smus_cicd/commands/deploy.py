...
def _display_deployment_info(target_name: str, target_config, manifest: PipelineManifest) -> None:
    """
...
def _deploy_bundle_to_target(target_config, manifest: PipelineManifest, config: Dict[str, Any]) -> bool:
    """
    ...
    if not storage_config and not workflows_config:
        handle_error("No storage or workflows configuration found in bundle_target_configuration")
        return False
    ...
    storage_result = _deploy_storage_files(bundle_file, storage_config, target_config.project.name, config)
    workflow_result = _deploy_workflow_files(bundle_file, workflows_config, target_config.project.name, config)
    ...
    if workflows_config:
        _validate_deployed_workflows(workflows_config, target_config.project.name, config)
...
def _find_bundle_file(manifest: PipelineManifest, config: Dict[str, Any]) -> Optional[str]:
    """
    ...
    return find_bundle_file(
        manifest.bundle.bundles_directory, 
    ...
    try:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            if os.path.exists(files_path):
                ...
                success = deployment.deploy_files(files_path, connection, '', region, files_path)
...
def _display_deployment_summary(
    ...
    try:
        ...
        if storage_files and storage_s3_uri:
            ...
            _add_files_to_tree(tree["📁 Storage Files"][f"📍 {storage_s3_uri}"], storage_files)
        
        # Add workflow files
        if workflow_files and workflow_s3_uri:
            tree["⚙️ Workflow Files"] = {f"📍 {workflow_s3_uri}": {}}
            _add_files_to_tree(tree["⚙️ Workflow Files"][f"📍 {workflow_s3_uri}"], workflow_files)
