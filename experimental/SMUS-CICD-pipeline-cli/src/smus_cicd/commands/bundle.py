    ...
    try:
        ...
        if 'domain' in manifest:
            ...
            if not config['region']:
                raise ValueError("Region must be specified in manifest domain configuration")
        ...
        if not target_name:
            ...
            if not target_name:
                typer.echo("Error: No target specified and no default target found", err=True)
                raise typer.Exit(1)
        ...
        from ..helpers.bundle_storage import ensure_bundle_directory_exists, upload_bundle, is_s3_url
        ...
        with tempfile.TemporaryDirectory(prefix="smus_bundle_") as temp_bundle_dir:
            ...
            if git_config:
                ...
                if url:
                    ...
                    try:
                        clone_path = os.path.join(temp_bundle_dir, target_dir.lstrip('./'))
                        os.makedirs(os.path.dirname(clone_path), exist_ok=True)
                        
                        result = subprocess.run(['git', 'clone', '--depth', '1', url, clone_path], 
                        ...
                        typer.echo(f"  Cloned {git_files_added} files from Git repository")
                        
                    except subprocess.TimeoutExpired:
                        typer.echo(f"Error: Git clone timed out after 60 seconds", err=True)
                    except Exception as e:
            ...
            if total_files_added > 0:
                ...
                final_bundle_path = upload_bundle(zip_path, bundles_directory, pipeline_name, region)
                
                file_size = os.path.getsize(zip_path)
                typer.echo(f"✅ Bundle created: {final_bundle_path} ({file_size} bytes)")
