...
def describe_command(
    manifest_file: str = typer.Argument("pipeline.yaml", help="Path to pipeline manifest file"),
    targets: str = typer.Option(None, "--targets", help="Filter to specific target name (optional, defaults to all targets)"),
    output: str = typer.Option("TEXT", "--output", help="Output format: TEXT (default) or JSON"),
    connections: bool = typer.Option(False, "--connections", help="Show connection information"),
    ...
    try:
        ...
        for target_name, target_config in targets_to_show.items():
            ...
            if hasattr(target_config, 'initialization') and target_config.initialization:
                target_data["initialization"] = {}
                if hasattr(target_config.initialization, 'project') and target_config.initialization.project:
                    ...
                    if hasattr(project_init, 'create'):
                        target_data["initialization"]["project"]["create"] = project_init.create
                    if hasattr(project_init, 'profile_name'):
                        target_data["initialization"]["project"]["profileName"] = project_init.profile_name
                    if hasattr(project_init, 'owners'):
                        target_data["initialization"]["project"]["owners"] = project_init.owners
                    if hasattr(project_init, 'contributors'):
                        target_data["initialization"]["project"]["contributors"] = project_init.contributors
                    if hasattr(project_init, 'userParameters'):
                        target_data["initialization"]["project"]["userParameters"] = project_init.userParameters
            ...
            if connections or connect:
                try:
                    ...
                    if project_info.get('project_id') and 'error' not in project_info:
                        ...
                        target_data["project"]["status"] = project_info.get('status', 'UNKNOWN')
                        ...
                        if output.upper() != "JSON":
                            ...
                            typer.echo(f"    Status: {project_info.get('status', 'UNKNOWN')}")
                            ...
                            if connections or connect:
                                ...
                                for conn_name, conn_info in project_connections.items():
                                    ...
                                    if conn_info.get('awsAccountId'):
                                        typer.echo(f"        awsAccountId: {conn_info['awsAccountId']}")
                                    if conn_info.get('description'):
                                        typer.echo(f"        description: {conn_info['description']}")
                                    
                                    # Display connection-specific properties
                                    conn_type = conn_info.get('type', '')
                                    if conn_type == 'WORKFLOWS_MWAA' and conn_info.get('environmentName'):
                                        typer.echo(f"        environmentName: {conn_info['environmentName']}")
                                    elif conn_type == 'ATHENA' and (conn_info.get('workgroup') or conn_info.get('workgroupName')):
                                        workgroup = conn_info.get('workgroup') or conn_info.get('workgroupName')
                                        typer.echo(f"        workgroup: {workgroup}")
                                    elif conn_type == 'S3' and conn_info.get('s3Uri'):
                                        typer.echo(f"        s3Uri: {conn_info['s3Uri']}")
                                    elif conn_type == 'LAKEHOUSE' and conn_info.get('databaseName'):
                                        typer.echo(f"        databaseName: {conn_info['databaseName']}")
                                    elif conn_type == 'SPARK':
                                        if conn_info.get('jobRole'):
                                            typer.echo(f"        jobRole: {conn_info['jobRole']}")
                                        if conn_info.get('permissionMode'):
                                            typer.echo(f"        permissionMode: {conn_info['permissionMode']}")
                                    
                                    # Display any other specific properties that might exist
                                    for key, value in conn_info.items():
                                        if key not in ['connectionId', 'type', 'region', 'awsAccountId', 'description', 
                                                     'environmentName', 'workgroup', 'workgroupName', 's3Uri', 'databaseName', 'jobRole', 'permissionMode']:
                        ...
                        if output.upper() != "JSON":
                            ...
                            should_create = getattr(target_config.project, 'create', False)
                            if should_create:
                                typer.echo(f"    ℹ️  Project will be created during deployment (create: true)")
                            else:
                                typer.echo(f"    ❌ Error getting project info: {project_info.get('error', 'Unknown error')}")
        ...
        if hasattr(manifest, 'workflows') and manifest.workflows and output.upper() != "JSON":
            typer.echo(f"\nManifest Workflows:")
            for workflow in manifest.workflows:
                ...
                typer.echo(f"    Trigger Post Deployment: {workflow.trigger_post_deployment}")
