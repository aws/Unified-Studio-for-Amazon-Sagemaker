...
def test_command(
    targets: str = typer.Option(None, "--targets", help="Target name(s) - single target or comma-separated list (optional, defaults to all targets)"),
    ...
    try:
        ...
        for target_name in target_list:
            ...
            if not os.path.exists(test_folder):
                ...
                test_results[target_name] = {"status": "error", "error": f"Test folder not found: {test_folder}"}
            ...
            if 'error' in project_info:
                ...
                test_results[target_name] = {"status": "error", "error": project_info['error']}
                overall_success = False
                _display_target_summary(target_name, test_results, output)
                continue
            ...
            if output.upper() != "JSON":
                ...
                typer.echo(f"  🔧 Project: {target_config.project.name} ({project_info.get('id', 'unknown')})")
        ...
        if output.upper() == "JSON":
            ...
            passed_targets = sum(1 for r in test_results.values() if r.get('status') == 'passed')
            failed_targets = sum(1 for r in test_results.values() if r.get('status') == 'failed')
            skipped_targets = sum(1 for r in test_results.values() if r.get('status') == 'skipped')
            error_targets = sum(1 for r in test_results.values() if r.get('status') == 'error')
