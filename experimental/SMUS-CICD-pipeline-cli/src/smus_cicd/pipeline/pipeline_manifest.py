...
class PipelineManifest:
    ...
    def from_dict(cls, data: Dict[str, Any]) -> 'PipelineManifest':
        ...
        for target_name, target_data in targets_data.items():
            if not target_data:
                raise ValueError(f"target '{target_name}' configuration cannot be empty")
            
            # Parse project config
            project_data = target_data.get('project')
            if not project_data:
                raise ValueError(f"target '{target_name}' must have a project configuration")
            
            if isinstance(project_data, str):
                # Handle simple string format: project: "project-name"
                if not project_data.strip():
                    raise ValueError(f"target '{target_name}' project name cannot be empty")
                project = ProjectConfig(name=project_data)
            else:
                # Handle object format
                project_name = project_data.get('name', '')
                if not project_name.strip():
                    raise ValueError(f"target '{target_name}' project.name is required and cannot be empty")
        ...
        for i, workflow_data in enumerate(workflows_data):
            ...
            if not workflow_name.strip():
                raise ValueError(f"workflow[{i}].workflowName is required and cannot be empty")
            
            connection_name = workflow_data.get('connectionName', '')
            if not connection_name.strip():
                raise ValueError(f"workflow[{i}].connectionName is required and cannot be empty")
            
            workflow = WorkflowConfig(
                workflow_name=workflow_name,
                connection_name=connection_name,
                trigger_post_deployment=workflow_data.get('triggerPostDeployment', False),
