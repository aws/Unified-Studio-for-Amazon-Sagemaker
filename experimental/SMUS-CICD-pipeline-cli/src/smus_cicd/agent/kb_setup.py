...
def deploy_kb_stack(region: str = 'us-east-1', user_role_arn: str = None, auto_approve: bool = False) -> Dict[str, Any]:
    ...
    try:
        ...
        while True:
            try:
                waiter.wait(
                    StackName=stack_name,
                    WaiterConfig={'Delay': 15, 'MaxAttempts': 1}
                )
                break
...
def create_opensearch_index(collection_endpoint: str, region: str, index_name: str = 'bedrock-knowledge-base-default-index'):
...
def create_bedrock_kb(role_arn: str, collection_arn: str, region: str) -> Dict[str, str]:
