...
class SMUSChatAgent:
    ...
    def __init__(self, model_id: str = None, kb_id: str = None):
        ...
        self.kb_retriever = (
            KnowledgeBaseRetriever(self.kb_id, region) if self.kb_id else None
        )
    ...
    def _show_welcome(self):
        ...
        if not self.kb_id:
            ...
            response = (
                input("\nWould you like to set up the Knowledge Base now? (yes/no): ")
                .strip()
                .lower()
            )
            if response in ["yes", "y"]:
                ...
                try:
                    ...
                    region = self.config.get("interactive", {}).get(
                        "region", "us-east-1"
                    )
                    ...
                    ds_id = create_data_source(
                        kb_result["kb_id"], outputs["BucketName"], region
                    )
