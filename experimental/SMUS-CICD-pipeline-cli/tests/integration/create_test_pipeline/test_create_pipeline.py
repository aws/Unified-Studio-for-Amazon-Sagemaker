...
class TestCreatePipeline(IntegrationTestBase):
    ...
    def test_create_pipeline_workflow(self):
        ...
        try:
            ...
            if result['success']:
                ...
                assert "create-test" in result['output'], f"Target 'create-test' not found in output"
            else:
            ...
            result = self.run_cli_command(["deploy", "--targets", "create-test", "--pipeline", pipeline_file])
            results.append(result)
            
            if result['success']:
                print("✅ Deploy command successful")
                assert "create-test-project" in result['output'], f"Project name not found in deploy output"
            else:
                print(f"⚠️ Deploy command failed (expected due to credentials): {result['error']}")
                # Check that it at least tried to deploy the right project
                assert "create-test-project" in result['output'] or "create-test-project" in result['error'], "Project name not found in deploy attempt"
            ...
            try:
                ...
                for param in user_params:
                    if param.get('EnvironmentConfigurationName') == 'Lakehouse Database':
                        lakehouse_param = param
                        break
                
                assert lakehouse_param is not None, "Lakehouse Database configuration not found"
                ...
                assert glue_db_param.get('value') == 'create_test_db', f"Expected 'create_test_db', got {glue_db_param.get('value')}"
                ...
                results.append({'success': True, 'output': "userParameters structure validated"})
                
            except Exception as e:
                print(f"❌ YAML validation error: {e}")
                results.append({'success': False, 'output': f"YAML validation error: {e}"})
        ...
        for i, result in enumerate(results, 1):
            ...
            print(f"  {status} Step {i}: {'Success' if result['success'] else 'Failed'}")
        ...
        if not critical_success:
            ...
            assert False, f"Critical commands must succeed: {[r for r in critical_commands if not r['success']]}"
        else:
    ...
    def test_userparameters_schema_validation(self):
        ...
        result = self.run_cli_command(["describe", "--pipeline", pipeline_file, "--output", "JSON"])
        ...
        try:
            ...
            assert 'userParameters' in project_init, "userParameters field should be present in JSON output"
            ...
            assert project_init.get('profileName') == 'All capabilities', "profileName should be 'All capabilities'"
            assert 'Eng1' in project_init.get('owners', []), "owners should contain 'Eng1'"
            
            print("✅ Schema validation test passed - initialization structure correctly present")
