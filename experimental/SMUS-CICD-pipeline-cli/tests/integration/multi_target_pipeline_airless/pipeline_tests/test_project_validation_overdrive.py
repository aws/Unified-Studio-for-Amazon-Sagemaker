...
def test_environment_variables_available():
    ...
    assert (
        "domain_name" in test_config["test_environment"]
    ), "domain_name should be in test_environment"
    assert (
        "project_prefix" in test_config["test_environment"]
    ), "project_prefix should be in test_environment"
    ...
    assert region in [
        ...
        "us-west-2",
        "eu-west-1",
    ], f"Region {region} should be valid"
...
def test_domain_and_project_ids():
    ...
    assert (
        test_project_name.replace("-", "").replace("_", "").isalnum()
    ), "Project name should be alphanumeric with dashes/underscores"
...
def test_aws_connectivity():
    ...
    try:
        ...
        try:
            ...
            print(
                f"✅ DataZone access available, found {len(domains.get('items', []))} domains"
            )
