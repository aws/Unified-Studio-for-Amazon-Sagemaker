...
class TestNewCommandsIntegration(IntegrationTestBase):
    ...
    def test_create_and_describe_workflow(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            describe_result = runner.invoke(
                app, ["describe", "--pipeline", manifest_file]
            )
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            for name in test_names:
                ...
                result = runner.invoke(
                    app, ["create", "--output", manifest_file, "--name", name]
                )
    ...
    def test_describe_target_filtering_comprehensive(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            create_result = runner.invoke(
                app,
                ["create", "--output", manifest_file, "--name", "TargetTestPipeline"],
            )
            assert create_result.exit_code == 0
            ...
            for target in targets:
                result = runner.invoke(
                    app, ["describe", "--pipeline", manifest_file, "--targets", target]
                )
    ...
    def test_run_command_parameter_validation(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            # Create manifest with specific output file
            create_result = runner.invoke(
                app, ["create", "--output", manifest_file, "--name", "RunTestPipeline"]
            )
            assert create_result.exit_code == 0
    ...
    def test_airflow_commands_variety(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            # Create manifest
            create_result = runner.invoke(
                app, ["create", "--name", "AirflowTestPipeline"]
            )
            assert create_result.exit_code == 0
            ...
            for command in airflow_commands:
                ...
                assert (
                    result.exit_code == 1
                ), f"Airflow command {command} should fail without AWS resources"
    ...
    def test_output_format_consistency(self):
        ...
        manifest_file = (
            Path(__file__).parent / "create_test_pipeline" / "create_test_pipeline.yaml"
        )

        # Test describe JSON format
        describe_json = runner.invoke(
            app, ["describe", "--pipeline", str(manifest_file), "--output", "JSON"]
        )
