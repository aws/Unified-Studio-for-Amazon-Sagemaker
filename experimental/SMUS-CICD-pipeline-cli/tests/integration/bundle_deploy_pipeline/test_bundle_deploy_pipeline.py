...
class TestBundleDeployPipeline(IntegrationTestBase):
                ...
                for root, dirs, files in os.walk(src_dir):
                    for file in files:
                        if not file.endswith(('.pyc', '.DS_Store')):
                            ...
                            arcname = os.path.join("storage", "src", os.path.relpath(file_path, src_dir))
    ...
    def test_describe_pipeline_without_dev(self):
        ...
        assert "Pipeline: BundleDeployPipeline" in result['output']
        assert "test: bundle-deploy-test" in result['output']
    ...
    def test_bundle_deploy_workflow(self):
        ...
        try:
            ...
            with zipfile.ZipFile(bundle_path, 'r') as zipf:
                ...
                assert any('workflows/dags/' in f for f in file_list), "No workflow files in bundle"
                assert any('storage/src/' in f for f in file_list), "No storage files in bundle"
                # Verify no src/src in paths
                assert not any('storage/src/src/' in f for f in file_list), "Found src/src in bundle paths"
            ...
            if result['success']:
                ...
                assert "Bundle file:" in result['output'], "Deploy output missing bundle file reference"
                assert "Deployment completed successfully!" in result['output'], "Deploy output missing success message"
            else:
            ...
            if result['success']:
                ...
                assert os.path.exists("/tmp/notebooks"), "Notebooks directory not created"
                notebook_files = os.listdir("/tmp/notebooks")
                assert any("covid_analysis" in f for f in notebook_files), "No covid analysis notebook downloaded"
            else:
            ...
            if result['success']:
                ...
                print(f"⚠️ Workflow execution failed (may be expected): {result['output']}")
        ...
        if len(results) > 0:
            ...
            assert deploy_result['success'], f"Bundle deployment must succeed: {deploy_result['output']}"
    ...
    def test_bundle_contents_validation(self):
        ...
        with zipfile.ZipFile(bundle_path, 'r') as zipf:
            ...
            for expected_file in expected_files:
                assert any(expected_file in f for f in file_list), f"Expected file not found in bundle: {expected_file}"
