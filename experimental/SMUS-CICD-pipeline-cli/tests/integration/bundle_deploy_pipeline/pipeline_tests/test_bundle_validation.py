...
class TestBundleValidation:
    ...
    def get_pipeline_file(self):
        ...
        return Path(__file__).parent.parent / "bundle_deploy_pipeline.yaml"
    ...
    def test_describe_connect_validation(self):
        ...
        assert result['returncode'] in [0, 1], f"Describe --connect command failed unexpectedly: {result['stderr']}"
        ...
        if result['success']:
            ...
            if "mwaa" in result['stdout'].lower():
                ...
                assert "project.workflow_mwaa:" in result['stdout'], "MWAA connection project.workflow_mwaa not found in connections list"
                assert "WORKFLOWS_MWAA" in result['stdout'], "WORKFLOWS_MWAA type not found - MWAA environment not properly configured"
                print("✅ MWAA environment is properly configured and available")
            else:
                print("❌ MWAA connection not found in describe output")
                assert False, "MWAA connection is required but not found in describe --connect output"
        else:
            # Expected failures: AWS connectivity, project not found, etc.
            expected_errors = ["connection", "project", "domain", "region", "mwaa", "environment"]
            assert any(err in result['stdout'].lower() or err in result['stderr'].lower() for err in expected_errors), \
    ...
    def test_deploy_command_validation(self):
        ...
        # Should succeed or fail gracefully
        assert result['returncode'] in [0, 1], f"Deploy command failed unexpectedly: {result['stderr']}"
        
        if result['success']:
            assert "bundle" in result['stdout'].lower() or "deploy" in result['stdout'].lower()
        else:
            # Expected failures: AWS connectivity, project not found, etc.
    ...
    def test_run_command_validation(self):
        ...
        # Test 1: Run version command with JSON output
        result = self.run_smus_command([
            "run",
            "--command", "version",
            "--pipeline", str(pipeline_file),
        ...
        # Should succeed or fail gracefully
        assert result['returncode'] in [0, 1], f"Run command failed unexpectedly: {result['stderr']}"
        
        if result['success']:
            assert "version" in result['stdout'].lower()
        else:
            # Expected failures: MWAA not available, project not found, etc.
            expected_errors = ["mwaa", "project", "connection", "workflow", "environment"]
            assert any(err in result['stderr'].lower() for err in expected_errors), \
                f"Unexpected run error: {result['stderr']}"
        
        # Test 2: Run workflows with JSON output using correct workflow name from manifest
        result2 = self.run_smus_command([
            "run",
            "--workflows", "execute_notebooks_dag",
            "--pipeline", str(pipeline_file),
            "--targets", "test",
            "--output", "JSON"
        ])
        ...
        # Should succeed or fail gracefully
        assert result2['returncode'] in [0, 1], f"Run workflows command failed unexpectedly: {result2['stderr']}"
        ...
        if result2['success']:
            ...
            # Expected failures: MWAA not available, project not found, etc.
            expected_errors = ["mwaa", "project", "connection", "workflow", "environment"]
            assert any(err in result2['stderr'].lower() for err in expected_errors), \
                f"Unexpected run workflows error: {result2['stderr']}"
    ...
    def test_test_command_validation(self):
        ...
        # Should succeed or fail gracefully
        assert result['returncode'] in [0, 1], f"Test command failed unexpectedly: {result['stderr']}"
        ...
        if result['success']:
            ...
            assert any(err in result['stdout'].lower() for err in expected_errors), \
    ...
    def test_monitor_command_validation(self):
        ...
        # Should succeed or fail gracefully
        assert result['returncode'] in [0, 1], f"Monitor command failed unexpectedly: {result['stderr']}"
        
        if result['success']:
            assert "monitor" in result['stdout'].lower() or "pipeline" in result['stdout'].lower()
        else:
            # Expected failures: MWAA not available, project not found, etc.
            expected_errors = ["mwaa", "project", "connection", "environment", "workflow"]
            assert any(err in result['stderr'].lower() for err in expected_errors), \
        ...
        # Should succeed or fail gracefully
        assert result2['returncode'] in [0, 1], f"Monitor workflows command failed unexpectedly: {result2['stderr']}"
    ...
    def test_bundle_structure_validation(self):
        ...
        with zipfile.ZipFile(bundle_path, 'r') as zipf:
            ...
            assert any('workflows/dags/' in f for f in file_list), "Missing workflow files"
            ...
            for expected_file in expected_files:
                assert any(expected_file in f for f in file_list), \
