...
def create_datazone_project(domain_name, project_name, region):
    ...
    try:
        ...
        try:
            profiles_response = datazone.list_project_profiles(domainIdentifier=domain_id)
...
def wait_for_project_ready(domain_id, project_id, region, max_wait=300):
    ...
    while time.time() - start_time < max_wait:
        try:
            ...
            deployment_details = project_response.get('environmentDeploymentDetails', {})
            overall_status = deployment_details.get('overallDeploymentStatus', 'UNKNOWN')
            
            print(f"📋 Project status: {project_status}, Deployment status: {overall_status}")
                ...
                try:
                    ...
                    for resource in provisioned_resources:
                        ...
                        if resource_name == 'userRoleArn':
                            print(f"✅ Found user role ARN in provisionedResources: {resource_value}")
                            
                            # Verify this matches the expected pattern
                            expected_pattern = f"datazone_usr_role_{project_id}_{env_id}"
                            if expected_pattern in resource_value:
                                print(f"✅ Role matches expected pattern: {expected_pattern}")
                            else:
                                print(f"⚠️ Role doesn't match expected pattern: {expected_pattern}")
...
def grant_lake_formation_permissions(database_name, role_arn, region, s3_bucket):
    ...
    try:
        ...
        try:
            ...
            try:
                ...
                print(f"✅ Granted wildcard table permissions for all tables in {database_name}")
            except Exception as e:
    ...
    try:
        ...
        for bucket in response['Buckets']:
            ...
            if domain_name.lower() in bucket_name.lower() and 'datazone' in bucket_name.lower():
                return bucket_name
        ...
        for _ in range(30):
            ...
            if status == 'SUCCEEDED':
                ...
                if len(rows) > 1:  # Skip header row
                    ...
                    if count > 0:
                        print(f"✅ Athena query succeeded for {table_name}: {count:,} records")
                        return True
                    else:
                ...
                print(f"❌ Athena query failed for {table_name}: {result['QueryExecution']['Status'].get('StateChangeReason', '')}")
                return False
...
def main():
    parser = argparse.ArgumentParser(description='Setup COVID-19 dataset for SMUS domain')
    ...
    project_info = create_datazone_project(args.domain_name, args.project_name, args.region)
    if not project_info:
        print("❌ Failed to create DataZone project")
        return
    
    # Wait for project to be fully ready
    if not wait_for_project_ready(project_info['domainId'], project_info['projectId'], args.region):
        print("❌ Project not ready within timeout, continuing anyway...")
    
    # Add Eng1 as project owner
    print(f"\n👤 Adding Eng1 as project owner...")
    add_project_member(project_info['domainId'], project_info['projectId'], 'Eng1', args.region)
    ...
    for csv_file in csv_files[:5]:  # Limit to first 5 files for testing
        try:
            ...
            # Create Glue table
            table_created = create_glue_table(table_name, args.database, s3_location, columns, args.region)
    ...
    project_role_arn = get_project_role_arn(project_info['domainId'], project_info['projectId'], args.region)
    if project_role_arn:
        grant_lake_formation_permissions(args.database, project_role_arn, args.region, bucket_name)
    
    # Create and run DataZone data source
    print(f"\n📊 Creating DataZone data source...")
    data_source_id = create_datazone_data_source(
        project_info['domainId'], 
