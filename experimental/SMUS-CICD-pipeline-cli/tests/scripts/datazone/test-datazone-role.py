...
def test_datazone_apis():
    ...
    region = "us-east-1"
    expected_role = (
        "arn:aws:iam::<ACCOUNT_ID>:role/datazone_usr_role_d8ipo2t2p8oalj_6hufhs8cbb5qjr"
    )
    ...
    try:
        ...
        if tooling_env_id:
            ...
            response = datazone.get_environment(
                domainIdentifier=domain_id, identifier=tooling_env_id
            )
    ...
    try:
        ...
        if tooling_env_id:
            response = datazone.get_environment_credentials(
                domainIdentifier=domain_id, environmentIdentifier=tooling_env_id
            )
