    ...
    for env in envs_response.get("items", []):
        ...
        try:
            targets_response = datazone.list_subscription_targets(
                domainIdentifier=domain_id, environmentIdentifier=env_id
            )
...
def create_subscription_target(
    domain_id, dev_env_id, target_config, dev_manage_role, target_manage_role, region
):
    ...
    try:
        ...
        response = datazone.create_subscription_target(
            ...
            authorizedPrincipals=[
                dev_manage_role
            ],  # Using dev role due to DataZone constraint
...
def create_subscription_grant(domain_id, subscription, target_id, dev_env_id, region):
    ...
    try:
        ...
        response = datazone.create_subscription_grant(
            ...
            environmentIdentifier=dev_env_id,
            grantedEntity={
                "listing": {"identifier": listing_id, "revision": listing_revision}
            },
...
def main():
    ...
    dev_manage_role = dev_target.get(
        "manageAccessRole"
    )  # Use dev project's role for manageAccessRole
    target_manage_role = target_target.get(
        "manageAccessRole"
    )  # Use target project's role for authorizedPrincipals
    ...
    if subscriptions:
        ...
        for subscription in subscriptions:
            ...
            listing_name = subscription.get("subscribedListing", {}).get(
                "name", "Unknown"
            )

            grant_id = create_subscription_grant(
                domain_id, subscription, new_target_id, dev_env_id, region
            )
