...
class TestDeployFailures:
    ...
    def test_deploy_nonexistent_manifest(self):
        ...
        runner = CliRunner()
        result = runner.invoke(app, ["deploy", "--pipeline", "nonexistent.yaml", "--targets", "test"])
    ...
    def test_deploy_invalid_manifest(self):
        ...
        try:
            result = runner.invoke(app, ["deploy", "--pipeline", invalid_manifest, "--targets", "test"])
            assert result.exit_code != 0
...
""")
        ...
        try:
            result = runner.invoke(app, ["deploy", "--pipeline", manifest_file, "--targets", "nonexistent"])
            assert result.exit_code != 0
            assert "not found" in result.stderr.lower() or "error" in result.stderr.lower()
        finally:
...
""")
        ...
        try:
            result = runner.invoke(app, ["deploy", "--pipeline", manifest_file, "--targets", "test"])
            assert result.exit_code != 0
            assert "not found" in result.stderr.lower() and "create=false" in result.stderr.lower()
        finally:
            os.unlink(manifest_file)
