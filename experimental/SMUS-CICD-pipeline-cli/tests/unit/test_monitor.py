...
def create_mock_manifest():
    ...
    mock_project_dev = type('MockProject', (), {
        'name': 'dev-project',
        'create': False
    })()
    
    mock_project_test = type('MockProject', (), {
        'name': 'test-project', 
        'create': False
    })()
    
    mock_target_dev = type('MockTarget', (), {
        'project': mock_project_dev
    })()
    
    mock_target_test = type('MockTarget', (), {
        'project': mock_project_test
    })()
    
    mock_manifest = type('MockManifest', (), {
        'pipeline_name': 'TestPipeline',
        'domain': mock_domain,
        'targets': {
            'dev': mock_target_dev,
            'test': mock_target_test
        },
        'workflows': []
    })()
...
def test_monitor_all_targets(mock_from_file, mock_get_project_info, mock_load_config, mock_mwaa_health, sample_manifest):
...
@patch("smus_cicd.commands.monitor.get_datazone_project_info")
@patch("smus_cicd.pipeline.PipelineManifest.from_file")
def test_monitor_single_target(mock_from_file, mock_get_project_info, mock_load_config, mock_mwaa_health, sample_manifest):
...
@patch("smus_cicd.commands.monitor.get_datazone_project_info")
@patch("smus_cicd.pipeline.PipelineManifest.from_file")
def test_monitor_inactive_project(mock_from_file, mock_get_project_info, mock_load_config, mock_mwaa_health, sample_manifest):
