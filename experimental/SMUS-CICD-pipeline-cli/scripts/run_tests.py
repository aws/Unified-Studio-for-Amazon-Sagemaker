...
def check_aws_setup():
    ...
    if aws_config.get('profile') or (aws_config.get('access_key_id') and aws_config.get('secret_access_key')):
        return True
...
def run_unit_tests(coverage=True, html_report=False):
    ...
    if html_report:
        cmd.extend(["--html=tests/reports/unit-test-results.html", "--self-contained-html"])
...
def run_integration_tests(coverage=True, html_report=False, skip_slow=False):
    ...
    if html_report:
        cmd.extend(["--html=tests/reports/integration-test-results.html", "--self-contained-html"])
...
def run_all_tests(coverage=True, html_report=False, skip_slow=False):
    ...
    if html_report:
        cmd.extend(["--html=tests/reports/all-test-results.html", "--self-contained-html"])
    
    print("🚀 Running all tests...")
    if not check_aws_setup():
        print("⚠️  Warning: AWS credentials not configured. Some integration tests may fail.")
...
def generate_coverage_report():
    ...
    subprocess.run(["python", "-m", "coverage", "xml", "-o", "tests/reports/coverage.xml"])
    ...
    if exit_code == 0:
        ...
        if html_report:
            ...
            if args.type == "unit":
                ...
                print(f"📊 Integration test results: tests/reports/integration-test-results.html")
            else:
